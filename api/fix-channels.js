...
export default async function handler(req, res) {
  try {
    ...
    const { rows: beforeFix } = await sql`
      SELECT 
        channel,
        COUNT(*) as count,
    ...
    const { rows: updateResult1 } = await sql`
      UPDATE bookings 
      SET channel = CASE
    ...
    const { rows: updateResult2 } = await sql`
      UPDATE bookings 
      SET channel = CASE
    ...
    const { rows: sampleBookings } = await sql`
